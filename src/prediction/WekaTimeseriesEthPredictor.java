 ...
 * @author andreads
 */
public class WekaTimeseriesEthPredictor {
    ...
    private static String[] conf = new String[]{
        ...
        "weka.classifiers.functions.LinearRegression -S 0 -C -R 1.0E-8 -num-decimal-places 4",
        "weka.classifiers.functions.LinearRegression -S 0 -C -R 1.0E-4 -num-decimal-places 4",
        "weka.classifiers.functions.LinearRegression -S 0 -C -R 1.0E-12 -num-decimal-places 4"
    };
    ...
    public static void main(String[] args) throws Exception {
        ...
        int confCounter = 0;
        for (String algorithm : conf) {
            String[] splitOptions = Utils.splitOptions(algorithm);
            String classname = splitOptions[0];
            splitOptions[0] = "";
            Classifier forName = (Classifier) Utils.forName(Classifier.class, classname, splitOptions);
            String confName = classname.substring(classname.lastIndexOf('.')+1)+"Conf" + (confCounter++);
            String fileOutputName = confName+"Result" + graphType + "Delta" + timeslotSize + "-" + timeslotConsecutivi + ".csv";
            ...
            for (String uid : result.keySet()) {
                ...
                //LOAD USER GRAPH
                //--------------------------
                HashMap<Long, HashMap<String, Double>> prepareValToPredict = loadGraphLight(result, uid);
                ...
                //TestWekaTimeseriesEthPredictor.predict(dataset);

                //File dove scrivere i risultati delle predizioni
                //File dove scrivere le proprieta dei nodie


                for (String target : fieldsToForecast) {
                    //LinearRegression function = new LinearRegression();
                    //IBk function = new IBk();
                    //function.setKNN(2);
                    //function.setNearestNeighbourSearchAlgorithm(new CoverTree());
                    ...
                    forecaster.setBaseForecaster(forName);
...
//                System.out.println("getRemoveLeadingInstancesWithUnknownLagValues: " + forecaster.getTSLagMaker().getRemoveLeadingInstancesWithUnknownLagValues());
                    ...
                    errors[0] = target;
                    errors[1] = String.valueOf(result.get(uid).isSC);
                    int index = 2;
     ...
     */
    private static HashMap<Long, HashMap<String, Double>> loadGraph(HashMap<String, UserInfo> result, String uid) {
     ...
     * Carico  dati degl utenti dai grafi
     */
    private static HashMap<Long, HashMap<String, Double>> loadGraphLight(HashMap<String, UserInfo> result, String uid) {
        //per ogni file
     ...
     * @throws IOException 
     */
    public static DirectedSparseMultigraph<NodeEth, Interaction> importG1(File fileName, int startBlock, int finalBlock, DirectedSparseMultigraph<NodeEth, Interaction> graph) throws IOException {
...
//                inte.setDst(to);
     ...
     * @throws IOException 
     */
    private static DirectedSparseMultigraph<NodeEth, Interaction> importG2(File fileName, int startBlock, int finalBlock, DirectedSparseMultigraph<NodeEth, Interaction> graph) throws IOException {
        CSVReader wr = new CSVReader(new FileReader(fileName), '\t', CSVWriter.DEFAULT_QUOTE_CHARACTER);
        String[] line;
...
//                inte.setDst(to);
    ...
    private static DirectedSparseMultigraph<NodeEth, Interaction> importG3(File fileName, int startBlock, int finalBlock, DirectedSparseMultigraph<NodeEth, Interaction> graph) throws IOException {
        CSVReader wr = new CSVReader(new FileReader(fileName), '\t', CSVWriter.DEFAULT_QUOTE_CHARACTER);
        String[] line;
        int lineNumber = 1;
        String prevTx = "";
...
//                    inte.setDst(to);
    ...
    private static HashMap<Long, HashMap<String, Double>> prepareValToPredict(HashMap<Long, DirectedSparseMultigraph<NodeEth, Interaction>> loadGraph, String uid, String[] prop) {
        ...
        for (long ts : loadGraph.keySet()) {
            HashMap<String, Double> prepareValToPredict = prepareValToPredict(loadGraph.get(ts), uid, prop);
            res.put(ts, prepareValToPredict);
        }
        return res;
    }

    public static HashMap<String, UserInfo> readUsersForPrediction() {
        //LEGGO tutti i file
        File dataDir = new File(dataDirName);
        if (!(dataDir.isDirectory())) {
            System.out.println("File " + dataDirName + " is not a dir..");
